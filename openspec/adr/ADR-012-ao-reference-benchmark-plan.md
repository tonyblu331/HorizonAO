...
- Three.js `GTAONode` is the current WebGPU/TSL AO baseline. It supports `radius`, `thickness`, `distanceFallOff`, `distanceExponent`, `scale`, `samples`, `resolutionScale`, optional temporal filtering with `TRAANode`, and optional spatial filtering in the example pipeline.
- The current Three.js `webgpu_postprocessing_ao.html` example uses a Tennyson bust room scene, half-resolution GTAO, packed unsigned-byte normals, velocity, and TRAA. The local benchmark route mirrors the scene layout but keeps temporal filtering off so raw debug rows and final product rows remain comparable.
- XeGTAO is archived by Intel as of 2024-04-22. The implementation is a DirectX/HLSL sample with three compute passes: depth prefilter/MIP generation, main AO, and spatial filtering. Treating XeGTAO as a single shader toggle would be technically wrong.
- XeGTAO's public guidance says its spatial filter is 5x5 and depth-aware, with TAA expected when available. It also uses full-resolution defaults, 3 slices, 6 samples per slice, Hilbert/R2 sampling, near-field falloff, and optional thin-occluder compensation.
...
Add `/museum` as the canonical comparison route before attempting a full XeGTAO or CACAO port.
...
- A single output toggle for raw debug versus final product semantics: Three baseline modes use their spatially filtered comparison output, while VBAO uses `VBAONode.getRawTextureNode()` for raw debug and `VBAONode.getTextureNode()` for final product AO.
- Live frame-time sampling in the overlay.

## Tradeoffs

- This is a fair visual harness, not a complete XeGTAO/CACAO port.
- The demo toggle is deliberately semantic, not an API promise: for VBAO it does not expose a public filter toolkit, it only switches between raw debug AO and the product node's final spatial output.
...
- The route is labeled Museum. `/museum` is the only active comparison route; no duplicate route is kept.
...
- Capture 1920x1080 and 1280x720 evidence for GTAO, VBAO, and N8AO in beauty and AO-only modes, with rows labeled as raw debug or final product where that distinction applies.
- If we port XeGTAO, implement it as compute-style passes: depth prefilter/MIPs, AO + edge data, spatial filter. Do not reduce it to a renamed GTAO preset.
