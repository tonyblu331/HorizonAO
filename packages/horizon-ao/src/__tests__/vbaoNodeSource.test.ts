...
describe('modernized VBAO production source contract', () => {
  ...
  it('keeps default VBAO temporal-free while allowing internal host phase animation', () => {
    ...
    expect(temporalGateSource).toContain('internalPrototypeAllowed')
    expect(temporalGateSource).toContain('internalTemporalAllowed: internalPrototypeAllowed')
    expect(temporalGateSource).toContain("'prototype-only'")
  ...
  it('exposes internal temporal diagnostics to benchmark evidence only', () => {
    ...
    expect(source).toContain('@internal Demo benchmark evidence hook')
    expect(source).toContain('getInternalTemporalDiagnostics()')
